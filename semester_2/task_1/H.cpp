...
Line::Line(int x1, int y1, int x2, int y2) : a_(y2 - y1), b_(x1 - x2), c_(y1* (x2 - x1) - x1 * (y2 - y1)) {
}
...
bool InPolygon() {
  ...
  Segment segment(x, y, 10003, 1);
  for (int64_t i = 0; i < n; ++i) {
    std::cin >> points[i];
  }
  for (int64_t i = 0; i < n - 1; ++i) {
    if (Segment(points[i].x_, points[i].y_, points[i + 1].x_, points[i + 1].y_).IsOnSegment(x, y)) {
      return true;
    }
  }
  if (Segment(points[0].x_, points[0].y_, points[n - 1].x_, points[n - 1].y_).IsOnSegment(x, y)) {
    return true;
  }
  int64_t count = 0;
  for (int64_t i = 1; i < n; ++i) {
    if (segment.SegmentSegmentCrossing(
      Segment(points[i - 1].x_, points[i - 1].y_, points[i].x_, points[i].y_))) {
      ++count;
    }
  }
  if (segment.SegmentSegmentCrossing(
    Segment(points[0].x_, points[0].y_, points[n - 1].x_, points[n - 1].y_))) {
    ++count;
  }
  return (count % 2 != 0);
}
